...
function getRedis() {
  ...
  // If the URL is provided but the token is missing, try to extract the token 
  // from the URL (format: https://[:token]@host)
  if (url && !token) {
    try {
      const parsed = new URL(url);
      if (parsed.password) {
        token = parsed.password;
        // Clean the URL to remove the credential part for the SDK
        parsed.username = '';
        parsed.password = '';
  ...
  if (!url || !token) {
    throw new Error(
      "Missing Redis/KV configuration. Please ensure you have linked a Vercel KV or Upstash store, or manually set REDIS_URL and REDIS_TOKEN."
    );
  }
